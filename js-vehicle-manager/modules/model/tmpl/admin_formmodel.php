...
<div id="jsvehiclemanageradmin-wrapper">
    ...
    <span class="jsvm_js-admin-title">
        <a href="<?php echo esc_url(admin_url('admin.php?page=jsvm_model&makeid='.jsvehiclemanager::$_data['makeid'])); ?>"><img src="<?php echo JSVEHICLEMANAGER_PLUGIN_URL; ?>includes/images/back-icon.png" /></a>
        <?php $msg = isset(jsvehiclemanager::$_data[0]) ? __('Edit', 'js-vehicle-manager') : __('Add New','js-vehicle-manager'); ?>
        <?php echo $msg . ' ' . jsvehiclemanager::$_data['maketitle']. ' ' .__('Model', 'js-vehicle-manager'); ?>
        <?php JSVEHICLEMANAGERincluder::getClassesInclude('jsvehiclemanageradminsidemenu'); ?>
    </span>
    <div class="jsvehiclemanager-form-wrap">
        <form id="jsvehiclemanager-form" method="post" action="<?php echo esc_url(admin_url("admin.php?page=jsvm_model&task=savemodel")); ?>">
            ...
            <?php echo wp_kses(JSVEHICLEMANAGERformfield::hidden('_wpnonce', wp_create_nonce('save-model')), JSVEHICLEMANAGER_ALLOWED_TAGS); ?>
            ...
            <div class="jsvm_js-submit-container">
                <div class="jsvm_js-button-container">
                    <a id="jsvm_form-cancel-button" href="<?php echo esc_url(admin_url('admin.php?page=jsvm_model&makeid='.jsvehiclemanager::$_data['makeid'])); ?>" ><?php echo __('Cancel', 'js-vehicle-manager'); ?></a>
                    <?php echo wp_kses(JSVEHICLEMANAGERformfield::submitbutton('save', __('Save','js-vehicle-manager') .' '. __('Model', 'js-vehicle-manager'), array('class' => 'button')), JSVEHICLEMANAGER_ALLOWED_TAGS); ?>
                </div>
            </div>
        </form>
    </div>
