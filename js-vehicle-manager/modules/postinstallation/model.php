...
class JSVEHICLEMANAGERPostinstallationModel {
    ...
    function installSampleData($insert_sample_data, $menu) {
        ...
        global $wpdb;
		$basepath = ABSPATH;
		$data_directory = JSVEHICLEMANAGERincluder::getJSModel('configuration')->getConfigValue('data_directory');
		if($data_directory == "") $data_directory = "jsvehiclemanager_data";
		$this->makeDir($wp_upload_dir["basedir"]."/".$data_directory);
        if($insert_sample_data == 1) {
            ...
            if (jsvehiclemanager::$_car_manager_theme == 0) {// eihter 4 vehicles or 26 vehicles
                ...
                $url = "https://wpvehiclemanager.com/setup/demoimporter/js_vehicle_manager/vm_sample_data.zip";
                $filepath = $basepath . "tmp/vm_sample_data.zip";
                ...
                if (file_exists($basepath . "/tmp/vm_sample_data.zip")) {
                    require_once ABSPATH . 'wp-admin/includes/class-pclzip.php';// using core library
                    $archive = new PclZip($basepath . "/tmp/vm_sample_data.zip");
    ...
    function importTemplateSampleData($flag){
        ...
        $url = "https://wpvehiclemanager.com/setup/demoimporter/js_vehicle_manager/cm_content_sample_data.zip";
        $filepath = $basepath . "/tmp/cm_content_sample_data.zip";
        ...
        if (file_exists($basepath . "/tmp/cm_content_sample_data.zip")) {
            require_once ABSPATH . 'wp-admin/includes/class-pclzip.php';// using core library
            $archive = new PclZip($basepath . "/tmp/cm_content_sample_data.zip");
        ...
        if($error_revslider == false){
            ...
            $url = "https://wpvehiclemanager.com/setup/demoimporter/js_vehicle_manager/rs_sample_data.zip";
            $filepath = $basepath . "/tmp/rs_sample_data.zip";

            // Download file
            $tmpfile = download_url( $url);
            copy( $tmpfile, $filepath );
            @unlink( $tmpfile ); // must unlink afterwards
            if(file_exists($basepath . "/tmp/rs_sample_data.zip")){ // Unzip
                $slider->importSliderFromPost(false, true, $basepath . "/tmp/rs_sample_data.zip");
            }
        }
    ...
    function storeSampleVehicleImages($vehicleid,$numberofimages,$count){
        ...
        $default = 1;
        $comma = ",";
        rename($wp_upload_dir["basedir"]."/".$data_directory."/data/vehicle/vehicle_x".$count,$wp_upload_dir["basedir"]."/".$data_directory."/data/vehicle/vehicle_".$vehicleid);
        for ($i=1; $i <= $numberofimages; $i++) {
            $query .='  ('.$vehicleid.', "'.$wp_upload_dir["baseurl"].'/'.$data_directory.'/data/vehicle/vehicle_'.$vehicleid.'/", "vehicleimage'.$i.'.jpg", "81494", "'.date("Y-m-d H:i:s").'", 1, '.$default.')'.$comma;
            $default = 0;
